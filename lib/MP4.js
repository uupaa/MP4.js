...
function _parseLoop(mp4box, view, parentNode) {
    ...
    while (view.cursor < loopEnd) {
        ...
        case "trak": _trak(mp4box, boxView, parentNode); break; // moov.trak is Array
...
function _ftyp(mp4box, boxView, parentNode) {
    // brand list: https://github.com/uupaa/MP4.js/wiki/Document#brand-list
...
function _trak(mp4box, boxView, parentNode) {
    var node = {};
    // multi track
    if (parentNode["trak"] && parentNode["trak"].length) {
        parentNode["trak"].push(node);
    } else {
